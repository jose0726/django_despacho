    ...
    async function init() {
        ...
        function abrirModal(p) {
            try {
                ...
                // Botón "Ver proyecto" → lleva a la página de detalle (con visor 3D si tiene modelo)
                const verBtn = document.createElement('a');
                verBtn.href = '/proyecto/' + p.id + '/';
                verBtn.className = 'btn-ver-proyecto';
                verBtn.textContent = 'Ver proyecto completo';
                verBtn.style.cssText = 'display:inline-block; margin-top:1rem; padding:0.6rem 1.4rem; background:#2563eb; color:#fff; border-radius:6px; text-decoration:none; font-size:0.95rem; font-weight:600; transition:background 0.2s;';
                verBtn.addEventListener('mouseenter', () => verBtn.style.background = '#1d4ed8');
                verBtn.addEventListener('mouseleave', () => verBtn.style.background = '#2563eb');
